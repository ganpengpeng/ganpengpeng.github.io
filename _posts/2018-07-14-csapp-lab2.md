...
从结果来看，可以知道这是一个链表。每个结点大小为16字节，前4个字节为结点值，第二个为结点编号，第三个为指向下一结点的指针，第四个均为0。
...
从 **401255** 处判断输入是一个数，程序会以字符串形式读取再使用strtol函数转换成long类型，估计这样做是为了迷惑拆弹人。

输入的数存放在rax寄存器中，接着判断输入的数减1后是否小于等于0x3e8(估计也是烟雾弹)。
...
- 将rdi指向的数取到edx中。
- 当edx等于esi时，eax赋0并返回。
- 当edx小于esi时，更新rdi指针(使rdi指向的数变大)，递归调用fun7，返回时eax乘以2再加1。
- 当edx大于esi时，更新rdi指针(使rdi指向的数变小)，递归调用fun7，返回时eax乘以2。
...
第一次在 **40126e** 处将 **0x6030f0** 送到edi(rdi的低32位)中，fun7地址 **740120d** 处的指令将rdi指向的数送到edx中，根据图中内容来看，edx为0x24。由于要求第一次edx先大于esi，所以esi小于0x24。接着 **401213** 处指令将rdi+8处内容送到rdi中，rdi变成 **0x603110** ，递归调用fun7。
